...
import { Sparkles, UserCheck, ShieldCheck, Briefcase, GraduationCap, LayoutDashboard } from 'lucide-react';
...
export const DemoRoleSwitcher: React.FC = () => {
  ...
  const handleSwitch = (targetRole: UserRole) => {
    ...
    toast.success(`Switched to ${targetRole.toUpperCase()} Dashboard!`);
  ...
  return (
    <div className="bg-gradient-to-r from-blue-900 via-indigo-900 to-slate-900 text-white text-xs py-2 px-4 flex flex-wrap items-center justify-between border-b border-blue-500/20 z-50 sticky top-0 shadow-md">
      <div className="flex items-center gap-2">
        ...
        <span className="font-semibold tracking-wide text-amber-300 uppercase">Demo Mode:</span>
        <span className="text-gray-300 hidden sm:inline">Switch views to evaluate all role dashboards:</span>
      </div>
      ...
      <div className="flex items-center gap-1.5 mt-1 sm:mt-0">
        ...
        <button
          ...
          className="flex items-center gap-1 px-2.5 py-1 rounded-md bg-amber-500/20 hover:bg-amber-500/40 text-amber-300 font-bold transition border border-amber-500/30"
        >
          <LayoutDashboard className="w-3.5 h-3.5" /> Learner Dashboard →
        </button>
      </div>
    </div>
  );
